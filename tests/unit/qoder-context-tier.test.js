...
function promptOfTokens(n) {
  ...
  return {
    system: "",
    messages: [{ role: "user", content: "abcd".repeat(n) }],
    tools: [],
  };
}
...
describe("getQoderContextTiers", () => {
  ...
  it("understands camelCase / snake_case variants and derives names", () => {
    const tiers = getQoderContextTiers({
      contextConfig: [
        { token_count: "400K", is_default: true },
...
describe("resolveQoderContextTier (auto)", () => {
  it("leaves the payload untouched while the prompt fits the current max_input_tokens", () => {
    expect(
      resolveQoderContextTier(MODEL_CONFIG, promptOfTokens(50_000)),
    ).toBeNull();
  });

  it("escalates to the smallest tier that fits once the prompt outgrows the default", () => {
    const choice = resolveQoderContextTier(
      MODEL_CONFIG,
      promptOfTokens(250_000),
    );
  ...
  it("applies headroom so a prompt just under the limit still escalates", () => {
    ...
    expect(
      resolveQoderContextTier(MODEL_CONFIG, promptOfTokens(170_000))?.tier.name,
    ).toBe("200K");
    // 190K * 1.15 = 218.5K → 200K no longer fits → 400K
    expect(
      resolveQoderContextTier(MODEL_CONFIG, promptOfTokens(190_000))?.tier.name,
    ).toBe("400K");
  });
...
describe("resolveQoderContextTier (forced via QODER_CONTEXT_TIER)", () => {
  ...
  it("a tier name or token count selects that tier", () => {
    expect(
      resolveQoderContextTier(MODEL_CONFIG, promptOfTokens(10), {
        preference: "400k",
      }).tier.tokenCount,
    ).toBe(400_000);
    expect(
  ...
  it("an unknown tier name falls back to auto", () => {
    ...
    expect(
      resolveQoderContextTier(MODEL_CONFIG, promptOfTokens(250_000), {
        preference: "9M",
      }).tier.name,
    ).toBe("400K");
  });
...
describe("applyQoderContextTier", () => {
  it("mirrors the tier into the three places the IDE writes", () => {
    const payload = {
      ...
      chat_context: {
        extra: { context: [], modelConfig: { key: "qmodel_38max" } },
      },
  ...
  it("lists visible models first, then hidden (enable:false) catalog keys", () => {
    const catalog = {
      ...
      rawConfigs: new Map([
        ...
        [
          "qfmodel",
          { key: "qfmodel", enable: false, display_name: "Qwen Fast" },
        ],
