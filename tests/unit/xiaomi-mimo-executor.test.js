...
import {
  XiaomiMimoExecutor,
  __test__,
} from "../../open-sse/executors/xiaomi-mimo.js";
...
describe("xiaomi-mimo executor", () => {
  ...
  it("routes Preview models to the account-service route regardless of transport", () => {
    ...
    expect(ex.buildUrl("xiaomi/mimo-x-flash-preview", true, 0, OPENAI_T)).toBe(
      expected,
    );
  });
  ...
  it("keeps the sourceFormat-matched endpoint for cloud models", () => {
    ...
    expect(ex.buildUrl("mimo-v2.5-pro", true, 0, CLAUDE_T)).toBe(
      CLAUDE_T.runtimeTransport.baseUrl,
    );
    expect(ex.buildUrl("mimo-v2.5-pro", true, 0, OPENAI_T)).toBe(
      OPENAI_T.runtimeTransport.baseUrl,
    );
  });
