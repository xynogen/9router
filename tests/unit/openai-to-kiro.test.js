...
describe("openaiToKiroRequest", () => {
  ...
  describe("thinking budget", () => {
    ...
    it.each([
      ...
      "maps GPT-5.6 reasoning.effort %s without legacy prompt tags",
      (effort, model) => {
        const body = {
          reasoning: { effort },
    ...
    it.each([
      ...
      "preserves GPT-5.6 effort %s as supported wire effort %s",
      (effort, model, wireEffort) => {
        const body = {
          reasoning: { effort },
    ...
    it.each([
      ...
      "normalizes unsupported Kiro intensity suffix for %s",
      (model, upstream) => {
        const result = openaiToKiroRequest(
          model,
          {
    ...
    it("defaults future Kiro model ids to additionalModelRequestFields support", () => {
      const body = {
        ...
        messages: [
          {
            role: "user",
            content: "Future model id should get adaptive fields",
          },
        ],
      };
    ...
    it("clamps reasoning_effort max to Kiro max_thinking_length 32000", () => {
      ...
      expect(result.additionalModelRequestFields?.output_config?.effort).toBe(
        "high",
      );
    });
    ...
    it("never emits top-level systemPrompt and keeps current time in user content", () => {
    ...
    it("replays frozen msg0 for explicit Kiro sessions while keeping current time fresh", () => {
      ...
      expect(second.conversationState).not.toHaveProperty(
        "agentContinuationId",
      );
