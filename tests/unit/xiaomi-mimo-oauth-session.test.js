...
vi.mock("open-sse/shared/mimoAccount.js", () => ({
  readDesktopPassToken: vi.fn(async () => ({
    passToken: "pt-abc",
    userId: "u1",
    cUserId: "c1",
  })),
...
vi.mock("@/lib/oauth/utils/server", () => {
  ...
  return {
    ...
    getXiaomiMimoSessionStatus: (state) => {
      ...
      return s
        ? { status: s.status, result: s.result || null, error: s.error || null }
        : null;
    },
...
const get = (action, state) =>
  GET(
    new Request(
      `http://localhost/api/oauth/xiaomi-mimo/${action}?state=${state}`,
    ),
    {
...
describe("xiaomi-mimo OAuth session lifecycle", () => {
  ...
  it("clears the session once /exchange consumed it", async () => {
    sessions.set("st1", {
      status: "done",
      result: { uid: "u1", accessToken: "sk-x" },
    });
    await exchange("st1");
