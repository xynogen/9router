...
describe("Claude tool `type` defaulting is provider-scoped (#3905)", () => {
  it("runs only for providers declaring requireClaudeToolType", () => {
    expect(
      shouldDefaultClaudeToolType("minimax", FORMATS.CLAUDE, tools, PROVIDERS),
    ).toBe(true);
    expect(
    ...
    expect(
      shouldDefaultClaudeToolType("deepseek", FORMATS.CLAUDE, tools, PROVIDERS),
    ).toBe(false);
    expect(
      shouldDefaultClaudeToolType("claude", FORMATS.CLAUDE, tools, PROVIDERS),
    ).toBe(false);
  });

  it("never applies outside Claude-format requests or without tools", () => {
    expect(
      shouldDefaultClaudeToolType("minimax", FORMATS.OPENAI, tools, PROVIDERS),
    ).toBe(false);
    expect(
      shouldDefaultClaudeToolType(
        "minimax",
    ...
    expect(
      shouldDefaultClaudeToolType("minimax", FORMATS.CLAUDE, null, PROVIDERS),
    ).toBe(false);
  });
