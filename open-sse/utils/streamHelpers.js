...
// Synthesize the terminal chunk(s) for a stream that ended without one.
// Strict clients hard-fail on this: pi-ai's openai-completions.js throws
// "Stream ended without finish_reason" if no chunk ever carried one, and
// Claude clients hang on a message that never gets message_stop.
// Returns an array of payloads ready for formatSSE(), or [] when the format
// needs no terminal (Responses has its own response.failed path; Gemini and
// Ollama clients tolerate a bare EOF).
// finishReason: "stop" for a clean EOF, "length" for abort/stall truncation.
export function buildTerminalChunks(
	sourceFormat,
	model,
	finishReason = "stop",
) {
