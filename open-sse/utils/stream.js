...
export function createSSEStream(options = {}) {
	...
	// Whether a chunk carrying a terminal marker reached the client. Strict clients
	// (pi-ai) throw "Stream ended without finish_reason" if the stream ends without one.
			...
			for (const line of lines) {
				...
				if (mode === STREAM_MODE.PASSTHROUGH) {
					...
					if (
						...
						try {
							...
							if (isFinishChunk) finishEmitted = true;
						} catch {
				...
				if (translated?.length > 0) {
					for (const item of translated) {
						...
						if (isFinishChunk || item.type === "message_stop")
							finishEmitted = true;
		...
		flush(controller) {
			...
			try {
				...
				if (mode === STREAM_MODE.PASSTHROUGH) {
					...
					// Synthesize a finish_reason chunk when upstream never sent one (truncated
					// stream, provider bug). Without it strict clients throw
					// "Stream ended without finish_reason" and discard content already received.
					if (!finishEmitted) {
						for (const terminal of buildTerminalChunks(FORMATS.OPENAI, model)) {
							const terminalOutput = formatSSE(terminal, FORMATS.OPENAI);
				...
				// Same guard as passthrough: close a stream the provider left dangling.
				// buildTerminalChunks returns [] for formats with their own terminal path
				// (Responses below) or that tolerate a bare EOF (Gemini family, Ollama).
				if (!finishEmitted) {
					for (const terminal of buildTerminalChunks(sourceFormat, model)) {
						const terminalOutput = formatSSE(terminal, sourceFormat);
						reqLogger?.appendConvertedChunk?.(terminalOutput);
						controller.enqueue(sharedEncoder.encode(terminalOutput));
					}
					finishEmitted = true;
