...
// Per-process session id, mirrors the official client's per-launch x-vscode-sessionid.
// ponytail: stable for the process lifetime; a real IDE rotates it per app launch.
const ANTIGRAVITY_PROCESS_SESSION_ID = crypto.randomUUID();
...
export class AntigravityExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials, stream = true, sessionId = null) {
    ...
    return {
      ...
      // Stable per-account machine id so it doesn't look like a new device each request.
      "x-machine-id": uuidFromSeed(
        `antigravity:machine:${credentials?.email || credentials?.connectionId || "anonymous"}`,
      ),
      "x-vscode-sessionid": ANTIGRAVITY_PROCESS_SESSION_ID,
    };
  }
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    // Match official client sampling defaults (upstream reference injects these).
    if (generationConfig.topK === undefined) generationConfig.topK = 40;
    if (generationConfig.topP === undefined) generationConfig.topP = 1.0;
