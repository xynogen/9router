...
function isKiroBody(body) {
  ...
  const historyTurn =
    ...
    cs.history.some(
      (it) => it && (it.userInputMessage || it.assistantResponseMessage),
    );
  return (
    historyTurn || !!(cs.currentMessage && cs.currentMessage.userInputMessage)
  );
}
...
function injectChatSystem(body, prompt) {
  try {
    ...
    try {
      idx = arr.findIndex(
        (m) => m && (m.role === ROLE.SYSTEM || m.role === ROLE.DEVELOPER),
      );
    } catch (_) {
      return;
    }
...
function injectGeminiSystem(body, prompt) {
  try {
    ...
    let useSnake = false;
    try {
      useSnake = Object.prototype.hasOwnProperty.call(
        target,
        "system_instruction",
      );
    } catch (_) {}
...
function injectKiroSystem(body, prompt) {
  try {
    ...
    const content =
      typeof targetMsg.content === "string" ? targetMsg.content : "";
