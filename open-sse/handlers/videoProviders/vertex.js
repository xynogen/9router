...
async function resolveAuth(credentials, log) {
  ...
  const location =
    credentials?.providerSpecificData?.location || DEFAULT_LOCATION;
...
function toVertexBody(body) {
  ...
  if (body.generate_audio != null)
    parameters.generateAudio = !!body.generate_audio;
...
export default {
  async buildRequest({
    ...
    if (action !== "generations") {
      ...
      return {
        error: `Vertex video supports 'generations' only (got '${action}')`,
      };
    }
    ...
    if (!/^[A-Za-z0-9._-]+$/.test(body.model))
      return { error: "Invalid Vertex video model id" };
