...
export function translateNonStreamingResponse(
	...
	if (sourceFormat === FORMATS.CLAUDE)
		return openAICompletionToClaudeMessage(openaiBody);
	if (sourceFormat === FORMATS.OPENAI_RESPONSES)
		return openAICompletionToResponsesApi(openaiBody);
...
function openAICompletionToResponsesApi(responseBody) {
	...
	const message = choice?.message || {};
	const output = [];
	const baseId = responseBody.id || Date.now();
	...
	for (let i = 0; i < toolCalls.length; i++) {
		...
		const callId = tc.id || `call_${baseId}_${i}`;
		output.push({
			type: "function_call",
			id: `fc_${callId}`,
			status: "completed",
