...
import {
 QODER_CONTEXT_TIER_HEADROOM,
 QODER_CONTEXT_TIER_MODES,
} from "./constants.js";
...
export function resolveQoderContextTier(modelConfig, prompt, options = {}) {
 ...
 const headroom =
  typeof options.headroom === "number"
   ? options.headroom
   : QODER_CONTEXT_TIER_HEADROOM;
 ...
 const fits = tiers.find(
  (t) => t.tokenCount >= need && t.tokenCount > currentLimit,
 );
...
export function applyQoderContextTier(payload, tier) {
 ...
 payload.parameters = {
  ...(payload.parameters || {}),
  context_length: tier.tokenCount,
 };
