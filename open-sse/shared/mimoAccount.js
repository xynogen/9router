...
function signatureClientSign(nonce, ssecurity) {
  const input =
    `nonce=${nonce}` + (ssecurity && ssecurity.trim() ? `&${ssecurity}` : "");
...
async function acquireServiceCookie(passJar, proxyOptions) {
  ...
  const nonce =
    j1.nonce ||
    (j1.location ? new URL(j1.location).searchParams.get("nonce") : null);
 ...
 */
export async function getMimoAccountCookie(
 ...
 */
export async function getMimoAccountUsage(
  providerSpecificData = null,
  proxyOptions = null,
) {
  const { cookie, reason } = await getServiceCookie(
    providerSpecificData,
    proxyOptions,
  );
  if (!cookie) {
    return {
      error: reason === "no-pass-token" ? "no-session" : "session-failed",
    };
  }
  try {
