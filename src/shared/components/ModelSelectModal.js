...
export default function ModelSelectModal({
  ...
  const liveConnectionIdsByProvider = useMemo(() => {
    const map = Object.fromEntries(
      LIVE_CATALOG_PROVIDERS.map((id) => [id, []]),
    );
  ...
  const cursorModels = useLiveProviderModels(
    isOpen,
    cursorConnectionIds,
    "Cursor",
  );
  const clineModels = useLiveProviderModels(
    isOpen,
    clineConnectionIds,
    "Cline",
  );
  const clinepassModels = useLiveProviderModels(
    isOpen,
    clinepassConnectionIds,
    "ClinePass",
  );
