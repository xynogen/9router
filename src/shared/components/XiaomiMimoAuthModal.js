...
export default function XiaomiMimoAuthModal({ isOpen, onSuccess, onClose }) {
    ...
    (async () => {
      ...
      try {
        ...
        if (data.found && data.apiKey) {
          ...
          setError(
            data.error ||
              "Xiaomi MiMo Desktop credentials not found on this machine.",
          );
        }
      } catch {
        if (!cancelled) {
  ...
  const handleStartOAuth = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/oauth/xiaomi-mimo/authorize?state=${state}`,
      );
      const data = await res.json();
  ...
  const handlePollOAuth = async () => {
    ...
    try {
      const res = await fetch(
        `/api/oauth/xiaomi-mimo/poll-status?state=${oauthState}`,
      );
      const data = await res.json();
      ...
      if (data.status === "done" && data.result) {
        ...
        setError(
          "Authorization not completed yet. Finish in the browser, then click Check Again.",
        );
      }
    } catch (err) {
      setError(err.message);
              ...
              <div className="flex gap-2 items-start">
                ...
                <div className="text-sm text-amber-800 dark:text-amber-200">
                  ...
                  <p className="mt-2 opacity-80">
                    Make sure Xiaomi MiMo Desktop is installed and you are
                    signed in, then retry. Or sign in via browser below.
                  </p>
                </div>
              </div>
            </div>
