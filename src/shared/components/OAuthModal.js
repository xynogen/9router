 ...
 */
function openOAuthWindow(url, target, features) {
  if (!url || typeof url !== "string") return null;
  const opener = window["open"];
  return typeof opener === "function" ? opener.call(window, url, target, features) : null;
}
...
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta, idcConfig }) {
  ...
  const startProxyFlow = useCallback(async (providerId) => {
    ...
    setStep("waiting");
    popupRef.current = openOAuthWindow(authData.authUrl, "oauth_popup", "width=600,height=700");
  ...
  const startOAuthFlow = async () => {
    ...
    try {
      ...
      if (deviceCodeProviders.includes(provider)) {
        ...
        if (verifyUrl) openOAuthWindow(verifyUrl, "_blank", "noopener,noreferrer");
      ...
      if (provider === "codex" && codexProxyActive) {
        ...
        setStep("waiting");
        popupRef.current = openOAuthWindow(data.authUrl, "oauth_popup", "width=600,height=700");
        if (!popupRef.current) {
          setStep("input");
        }
        ...
        setStep("input");
        openOAuthWindow(data.authUrl, "_blank");
      } else {
              ...
              <div className="bg-sidebar p-4 rounded-lg mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    icon="open_in_new"
                    onClick={() => openOAuthWindow(deviceLoginUrl, "_blank", "noopener,noreferrer")}
                    disabled={!deviceLoginUrl}
                  >
                    Open
                  </Button>
