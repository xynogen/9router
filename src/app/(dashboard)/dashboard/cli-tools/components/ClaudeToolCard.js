...
export default function ClaudeToolCard({
  ...
  const handleOneMContextToggle = (enabled) => {
    ...
    tool.defaultModels.forEach((model) => {
      ...
      if (current)
        onModelMappingChange(model.alias, withContextMarker(current, enabled));
    });
  };
  ...
  useEffect(() => {
    ...
    if (!env) return;
    setOneMContext(
      tool.defaultModels.some((model) => env[model.envKey]?.endsWith("[1m]")),
    );
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer select-none">
                    <input
                      ...
                      checked={oneMContext}
                      onChange={(e) =>
                        handleOneMContextToggle(e.target.checked)
                      }
