...
export async function GET(request) {
  try {
    ...
    // ponytail: single flag opt-out for trusted single-user self-hosted setups.
    // Ceiling: this exposes ALL users' payloads — only safe when you're the only
    // user. Upgrade path: gate on an admin/owner session instead of a global env.
    if (process.env.USAGE_SHOW_PAYLOADS === "1") {
