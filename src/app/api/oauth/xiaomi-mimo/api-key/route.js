...
export async function POST(request) {
  try {
    const { apiKey, uid, baseUrl, mimoPassToken, mimoUserId, mimoCUserId, region } =
      await request.json();
      ...
      (c) =>
        ...
        ((uid && c.email === `${uid}@xiaomi`) ||
          ...
          (sessionOnly &&
            ...
            (normRegion
              ? (c.providerSpecificData?.region || "cn") === normRegion
              : true))),
    );
    if (existing) {
      const updated = await updateProviderConnection(existing.id, {
        ...
        providerSpecificData: {
          ...
          mimoPassToken:
            mimoPassToken ||
            existing.providerSpecificData?.mimoPassToken ||
            null,
          mimoUserId:
            mimoUserId || existing.providerSpecificData?.mimoUserId || null,
          mimoCUserId:
            mimoCUserId || existing.providerSpecificData?.mimoCUserId || null,
          modelCount,
        },
